...
public:
    ...
    bool begin(const char* robotName = NULL);
    void end();
    void sleep();
...
// UNDONE: Might just want to find Arduino code for Serial class and its subclasses and implement them here.
class MiPStream
{
public:
    MiPStream() { }

    void println(const char* pString) { printf("%s\n", pString); }
    void println(uint32_t value, int base = 10) { print(value, base); printf("\n"); }
    void print(const char* pString) { printf("%s", pString); }
    void print(uint32_t value, int base = 10)
    {
        switch (base)
        {
            case 16:
...
// Things that would normally be found in Arduino/AVR headers.
#define HEX 16
#define DEC 10
