...
bool MiP::begin(const char* robotName /* = NULL */)
{
    ...
    int result = mipTransportConnectToRobot(m_pTransport, robotName);
    ...
    m_lastRequestTime = mipTransportGetMilliseconds(m_pTransport) - MIP_REQUEST_DELAY;
    m_lastContinuousDriveTime = mipTransportGetMilliseconds(m_pTransport) - MIP_CONTINUOUS_DRIVE_DELAY;
    ...
    {
        // UNDONE: Don't need to do this over BLE.
        ...
        mipTransportDelayMilliseconds(m_pTransport, 30);
        // Flush any outstanding junk data in receive buffer.
        mipTransportDiscardUnusedBytes(m_pTransport);
        ...
        mipTransportDelayMilliseconds(m_pTransport, MIP_BEGIN_RETRY_WAIT);
    }
...
void MiP::verifiedSetGestureRadarMode(MiPGestureRadarMode desiredMode)
{
    ...
    for (uint8_t retry = 0 ; retry < MIP_MAX_RETRIES ; retry++)
    {
        ...
        mipTransportDelayMilliseconds(m_pTransport, MIP_RETRY_WAIT);
    }
...
void MiP::continuousDrive(int8_t velocity, int8_t turnRate)
{
    ...
    if (mipTransportGetMilliseconds(m_pTransport) - m_lastContinuousDriveTime < MIP_CONTINUOUS_DRIVE_DELAY)
    {
        m_lastError = MIP_ERROR_NONE;
        return;
    }
    m_lastContinuousDriveTime = mipTransportGetMilliseconds(m_pTransport);
...
void MiP::transportSendRequest(const uint8_t* pRequest, size_t requestLength, int expectResponse)
{
    ...
    while (mipTransportGetMilliseconds(m_pTransport) - m_lastRequestTime < MIP_REQUEST_DELAY)
    {
    }
    ...
    mipTransportSendBytes(m_pTransport, pRequest, requestLength);

    m_lastRequestTime = mipTransportGetMilliseconds(m_pTransport);
}
...
int8_t MiP::transportGetResponse(uint8_t* pResponseBuffer, size_t responseBufferSize, size_t* pResponseLength)
{
    ...
    uint32_t startTime = mipTransportGetMilliseconds(m_pTransport);
    bool responseFound = false;
    do
    {
        responseFound = processAllResponseData();
    } while (!responseFound && mipTransportGetMilliseconds(m_pTransport) - startTime < MIP_RESPONSE_TIMEOUT);
    if (!responseFound)
    {
...
bool MiP::processAllResponseData()
{
    ...
    while (mipTransportResponseBytesAvailable(m_pTransport) >= 2)
    {
        uint8_t highNibble;
        uint8_t lowNibble;
        uint8_t commandByte;
        mipTransportReceiveByte(m_pTransport, &highNibble);
        mipTransportReceiveByte(m_pTransport, &lowNibble);
        ...
        if (m_expectedResponseCommand != 0 && commandByte == m_expectedResponseCommand)
        {
            ...
            mipTransportReceiveBytes(m_pTransport, buffer, bytesToRead * 2, &bytesRead);
...
void MiP::processOobResponseData(uint8_t commandByte)
{
    ...
    case MIP_CMD_GET_STATUS:
        ...
        break;
    default:
        uint8_t discardedBytes = mipTransportDiscardUnusedBytes(m_pTransport);
    ...
    uint8_t buffer[2 * 2];
    uint8_t bytesRead;
    mipTransportReceiveBytes(m_pTransport, buffer, length * 2, &bytesRead);
...
// Instantiate the single instance of the fake Serial stream for MiP coding on macOS.
